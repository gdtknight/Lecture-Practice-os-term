...
int fs_format()
{
	...
	if (sizeof(DIRENTRY_T) % 64) {
		printf("DIR_ENTRY should be fit in 64 byte boundary, cur size = %d\n", sizeof(DIRENTRY_T));
		exit(1);
	}
	...
	sectors = Disk_Stat();
	FAT_start = 1;
	num_FAT_sectors = (sectors * sizeof(unsigned short)) / 512;
	if ((sectors * sizeof(unsigned short)) % 512) num_FAT_sectors++;
...
int	fs_dir()
{
	...
	for (i = 0; i < files_in_dir; i++) {
		Disk_ReadSector(sector, buf);
		ep = (DIRENTRY_T *) (buf+offset);
		if (ep->status == DIR) printf("  %14s/ %d", ep->name, ep->size);
		else printf("  14%s  %d", ep->name, ep->size);	/* ep->status == FILE */
